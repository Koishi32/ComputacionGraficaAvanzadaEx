...
bool iniciaPartida =false, precionarOpcion=false;
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	// Se inicializa el modelo de render text
	modelText = new FontTypeRendering::FontTypeRendering(screenWidth,screenWidth);
...
bool processInput(bool continueApplication) {
	...
	if(!iniciaPartida){
		...
		}else if(!precionarOpcion && glfwGetKey(window,GLFW_KEY_LEFT_CONTROL) == GLFW_PRESS){
			precionarOpcion = true;
...
void applicationLoop() {
	...
	while (psi) {
		...
		shaderMulLighting.setVectorFloat3("directionalLight.light.ambient", glm::value_ptr(glm::vec3(0.8, .8, .8)));
		...
		shaderTerrain.setVectorFloat3("directionalLight.light.ambient", glm::value_ptr(glm::vec3(0.8, .8, .8)));
		...
		/****************************Update position objetos Transparencia ********************/
		//Actualizar Aircraft lambo Heli
		...
		/*******Ordenar los objetos******Conforme a la pocision del observador*/
		...
		glDisable(GL_CULL_FACE); // Se activa para ahorrar recursos, 
		std::map<float,std::pair<std::string,glm::vec3>>:: reverse_iterator itblendSorted = blendingSorted.rbegin(); // cambiar orden con reverse
		...
		glEnable(GL_BLEND);
		modelText -> render("Texto en OpenGL", -1,0,0,1,0,35); //coordenadas donde aparecera 
